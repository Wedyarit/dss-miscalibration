...
import { AnalyticsOverview, ReliabilityResponse, ProblematicItem } from '@/lib/types'
      ...
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  <span className="font-medium">
                    {t("instructor.modelInfo.lastUpdated")}
                  </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard
            ...
            variant={overview && overview.ece < 0.1 ? 'success' : overview && overview.ece > 0.2 ? 'danger' : 'warning'}
          />
          <MetricCard
            title={t('instructor.metrics.brier')}
            value={overview ? overview.brier.toFixed(3) : 'N/A'}
            description={t('instructor.metrics.lowerIsBetter')}
            variant={overview && overview.brier < 0.2 ? 'success' : overview && overview.brier > 0.4 ? 'danger' : 'warning'}
          />
          <MetricCard
            ...
            variant={overview && overview.roc_auc > 0.8 ? 'success' : overview && overview.roc_auc < 0.6 ? 'danger' : 'warning'}
          />
          <MetricCard
            ...
            variant={overview && overview.confident_error_rate < 0.2 ? 'success' : overview && overview.confident_error_rate > 0.4 ? 'danger' : 'warning'}
          />
        </div>
        ...
        {problematicItems.length > 0 && (
          <DataTable
            ...
            headers={t('instructor.tableHeaders')}
          />
        )}
          ...
          <Card>
            <CardContent className="p-8 text-center">
              <p className="text-muted-foreground mb-4">
                {t("instructor.noData.message")}
              </p>
              <Button asChild>
                <a href="/student/test">
                  {t("instructor.noData.startTest")}
                </a>
              </Button>
            </CardContent>
          </Card>
