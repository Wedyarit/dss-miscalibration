...
import {AnswerResponse, PredictionResponse, Question} from '@/lib/types'
...
export default function StudentTestPage() {
    ...
    const startTest = async () => {
        try {
            setIsLoading(true)
            const newSession = await sessionsApi.create(1, mode) // Demo user ID
    ...
    const loadNextQuestion = async () => {
        try {
            ...
            // Используем API для получения вопроса с учетом session_id
            ...
            // Если 404 - все вопросы завершены
    ...
    const handleAnswer = async (chosenOption: number, confidence?: number, responseTime?: number) => {
        ...
        try {
            ...
            // Get prediction only in standard mode and if user hasn't returned to question yet
            if (mode === 'standard' && !hasReturnedToQuestion) {
                ...
                // In self_confidence mode or after return, no prediction/risk badge
                setPrediction(null)
    ...
    const handleContinue = () => {
        setPrediction(null)
        setCanReturnToQuestion(false)
        // Показываем правильный ответ и переходим к следующему вопросу
    }
                    ...
                    <div className="space-y-6">
                        ...
                        <Card className="w-full max-w-md mx-auto">
                            <CardContent className="p-8 text-center space-y-4">
                                <h3 className="text-lg font-semibold">{t('studentTest.modeSelection.readyToStart')}</h3>
                                ...
                                <Button
                                    ...
                                    className="w-full"
                                >
                                    {isLoading ? t('studentTest.modeSelection.starting') : t('studentTest.modeSelection.startTest')}
                                </Button>
                            </CardContent>
                        </Card>
                    </div>
                            ...
                            <Card>
                                <CardHeader>
                                    <CardTitle className="flex items-center justify-between">
                                        <span>{t('studentTest.session.sessionNumber').replace('{id}', session.id.toString())}</span>
                                        <div className="flex items-center space-x-2">
                                            <Badge
                                                variant="outline">{session.mode === 'standard' ? t('studentTest.modeSelection.standard') : t('studentTest.modeSelection.selfConfidence')}</Badge>
                                            <Badge
                                                variant="secondary">{t('studentTest.session.questions').replace('{count}', questionCount.toString())}</Badge>
                                        </div>
                                    </CardTitle>
                                </CardHeader>
                            </Card>
                                    ...
                                    <CardTitle className="flex items-center space-x-2">
                                        ...
                                        {(!prediction || prediction.risk < 0.5 || !canReturnToQuestion) && (
                                            <Badge variant={answerFeedback.is_correct ? "default" : "destructive"}>
                                                {answerFeedback.is_correct ? t('studentTest.feedback.correct') : t('studentTest.feedback.incorrect')}
                                            </Badge>
                                        )}
                                    </CardTitle>
                                </CardHeader>
